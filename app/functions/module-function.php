...
function createSitemap(){
  ...
  ->join('menu_type','menu.menu_type_id', '=' ,'menu_type.id');
  $query->where('menu_type.theme_location','main-menu');  
  $query->select('menu.alias');
  $data_menu=$query->get()->toArray();
  $data_menu=convertToArray($data_menu);
  $data_article=ArticleModel::whereRaw('status = 1')->select('alias')->get()->toArray();
  $data_product=ProductModel::whereRaw('status = 1')->select('alias')->get()->toArray();
  $data_project=ProjectModel::whereRaw('status = 1')->select('alias')->get()->toArray();
  $data_organization=OrganizationModel::whereRaw('status = 1')->select('alias')->get()->toArray();
  $data_project_article=ProjectArticleModel::whereRaw('status = 1')->select('alias')->get()->toArray();
...
<sitemapindex xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/siteindex.xsd">';
  ...
  if(count($data_menu) > 0){
      ...
      $loc='<loc>'.url($value['alias'].'.html').'</loc>' ;      
      $url.='<sitemap>'.$loc.'</sitemap>' ;
    }
  }  
  ...
  if(count($data_project_article) > 0){
      foreach ($data_project_article as $key => $value) {
      $loc='<loc>'.url($value['alias'].'.html').'</loc>' ;      
      $url.='<sitemap>'.$loc.'</sitemap>' ;
    }
  }
